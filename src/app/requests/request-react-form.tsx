...
export default function RequestReactForm({
    ...
    const [createRequest, { data: createRequestResponse, loading, error }] =
        useMutation(CREATE_REQUEST, {
            ...
            onCompleted: data => {
                console.log(
                    'CreateRequest() completed with: ' + JSON.stringify(data),
                );
                form.clearErrors();
                form.reset({
                    name: '',
                    ...
                    description: (
                        <pre className="mt-2 w-auto rounded-md bg-slate-950 p-4">
                            <p className="text-green-700">
                                ...
                                {data.createRequest.request.id}, Name:{' '}
                                {data.createRequest.request.name}, Address:{' '}
                                {
                                    data.createRequest.request.address
                                        .properties?.addressString
                                }{' '}
                                {
                                    data.createRequest.request.address.geometry
                                        .coordinates[0]
                                }
                                ,
                                {
    ...
    function onSubmit(submission: z.infer<typeof FormSchema>) {
        ...
        const mutation = { variables: { input: submission } };
        // Override and reformat tags as simple String[] and lowercase them
        (mutation.variables.input.tags as unknown as string[]) =
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="outline">ADD NEW PLACE</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Create a new request</DialogTitle>
                    <DialogDescription>
                        This action will create a request for a place to be
                        added to the map. Your request will be reviewed and
                        confirmed by Admin in 3-5 days.
                    </DialogDescription>
                </DialogHeader>
